...
class JobPost extends Component {
  ...
  handleJobTitleChange = (job) => {
    ...
    this.handleJobPostChange({ jobPostId: id }).then((res) => {
      this.handleJobPostChange({ title: job.title?.suggestion })
        .then((res) => {})
        .catch((error) => {
          console.log(error?.response?.data);
        })
  ...
  onSubmit = async (e) => {
    e.preventDefault();
    const { checked, removeJobPost, jobPost, originalJobPost, newJobPost } =
      this.state;
    if (!checked) {
    ...
    if (removeJobPost) {
      ...
      const isChangedJob =
        originalJobPost && originalJobPost.title !== jobPost.title;
  ...
  searchJob = async (newValue) => {
    ...
    await axios
      .get(url, { params: { contains: newValue } })
      .then((response) => {
    ...
    return (
      <div className="container ">
        <form action="" onSubmit={this.onSubmit}>
          <label className="label-jp">
            Company name <span className="mandatory">*</span>
          </label>
          <input
            type="text"
            onChange={(e) =>
              this.handleJobPostChange({ companyName: e.target.value })
            }
          ...
          <label className="label-jp">
            Job <span className="mandatory">*</span>
          </label>
          <Autocomplete
            id="combo-box-demo"
            ...
            onInputChange={debounce(
              (event, value) => this.searchJob(value),
              100
            )}
          ...
          <input
            type="text"
            onChange={(e) =>
              this.handleJobPostChange({ employment: e.target.value })
            }
            className="form-control"
          ...
          <Autocomplete
            ...
            onInputChange={(event, value) => {
              this.loadPlaceOptions(value);
              this.handleJobPostChange({ location: value });
            }}
          ...
          <textarea
            ...
            rows="6"
            onChange={(e) =>
              this.handleJobPostChange({ description: e.target.value })
            }
          ...
          <p>
            <span className="mandatory">*Required fields</span>
          </p>
