      ...
      <div className="container ">
        <DialogTitle id="form-dialog-title" className="pb-3">
          ...
          {isMyProfile && (
            <IconButton className="edit-contact-info pt-0">
              <EditIcon color="action" onClick={this.onChangeContactInfo} />
            </IconButton>
          )}
          <h5>Contact Info</h5>
        </DialogTitle>
            ...
            {user.websites.map((url) => {
              return (
                <div key={uuid()} className="row pl-4">
                  <a className="ml-1" href={`${url}`} target="_blank">
                    {url}
                  </a>
                </div>
              );
        ...
        >
          <DialogContent>
            ...
            <ChangePersonalInfos user={user} onEdit={this.onSubmitInfo} />
          </DialogContent>
        </Dialog>
      </div>
    );
