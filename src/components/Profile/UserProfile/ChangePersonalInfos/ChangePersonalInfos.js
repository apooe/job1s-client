    ...
    await axios
      ...
      .then((res) => {
        this.handleUserChange({
          relatedJobs: res?.data?.related_job_titles.map((j) => j.title),
        });
      })
      .catch((error) => {
        console.log(error?.response?.data);
      ...
      <div className="container">
        ...
        <form action="" onSubmit={this.onSubmit}>
          ...
          <input
            type="text"
            onChange={(e) =>
              this.handleUserChange({ lastname: e.target.value })
            }
            className="form-control"
            ...
            websites.map((url) => {
              return (
                <div key={uuid()} className="p-0 m-0">
                  ...
                  <a className="m-0 p-0 ml-1" href={`${url}`}>
                    {url}
                  </a>
                  <IconButton
                    className="ml-2 "
          ...
          <a href="#" className="p-0 mt-2 d-flex" onClick={this.onAddWebsite}>
            <AddIcon color="primary" /> Add website
          </a>
