...
class ProfileUser extends Component {
  ...
  handleExperienceSubmit = (newExperience) => {
    ...
    if (isNewExperience) {
      newProfile.experience = newProfile?.experience
        ? [...newProfile.experience, newExperience]
        : [newExperience];
    } else {
      newProfile.experience = newProfile?.experience.map((exp) =>
        exp.companyName === this.state.originalSelectedExperience.companyName &&
        exp.startDate === this.state.originalSelectedExperience.startDate
          ? newExperience
          : exp
      );
    }
  ...
  handleEducationSubmit = (newEducation) => {
    ...
    if (isNewEducation) {
      ...
      newProfile.education = newProfile?.education.map((educ) =>
        educ.collegeName === this.state.originalSelectedEducation.collegeName
          ? newEducation
          : educ
      );
    }
  ...
  handleDelete = (dataToDelete, type) => {
    ...
    if (type === EXPERIENCE_ARRAY) {
      newProfile.experience = newProfile.experience.filter(
        (exp) => exp !== dataToDelete
      );
      this.setState({
        profile: newProfile,
        selectedExperience: null,
      ...
      newProfile.education = newProfile.education.filter(
        (educ) => educ !== dataToDelete
      );
      this.setState({
        profile: newProfile,
        selectedEducation: null,
  ...
  render() {
    ...
    const profilePictureImg = profile?.profileImg
      ? `${profile?.profileImg}`
      : picImage;
    return (
      <div>
        {profile ? (
          <div className="container mt-5">
              ...
              <div className="col-12">
                <section className="presentation-section border rounded p-2">
                  <div
                    className="picture"
                    onClick={() =>
                      this.state.onChangeInfo && this.uploadImg.current.click()
                    }
                  >
                  ...
                  {this.state.isMyProfile && (
                    <div className="row">
                      <div className="col-12">
                        {this.state.onChangeInfo ? (
                          <div className=" float-right p-2 text-info">
                            <button
                          ...
                          <IconButton
                            aria-label="edit"
                            className="float-right p-2 text-info"
                            onClick={this.onClickUpdateInfos}
                          >
                            <EditIcon>fontSize="small" Update profile</EditIcon>
                          </IconButton>
                        )}
                      </div>
                    </div>
                    ...
                    {profile.education &&
                      profile.education.map((formation, index) => (
                        <div className="mt-2 pl-2" key={uuid()}>
                          <h4 className="company-educ-name">
                            <a href={formation.link} target="_blank">
                              {formation.collegeName}
                            </a>
                          </h4>
                          {this.state.onChangeInfo && (
                          ...
                          <p className="date">
                            {" "}
                            {this.formatDate(formation.startDate)} -{" "}
                            {this.formatDate(formation.endDate)}{" "}
                          </p>
                          ...
                          {this.state.onChangeInfo && (
                            <div style={{ float: "right" }}>
                              <IconButton
                                ...
                                className="text-info"
                                onClick={() =>
                                  this.updateDataArray(exp, EXPERIENCE_ARRAY)
                                }
                              >
                                <EditIcon fontSize="small"></EditIcon>
                              </IconButton>
                              <IconButton
                                ...
                                className="text-danger"
                                onClick={() =>
                                  this.deleteDataArray(exp, EXPERIENCE_ARRAY)
                                }
                              >
                                <DeleteIcon fontSize="small"></DeleteIcon>
                              </IconButton>
            ...
            <Dialog
              open={this.state.onChangeInfo && this.state.onChangeExperiences}
              onClose={this.updateDataArray}
              aria-labelledby="form-dialog-title"
            >
              <DialogTitle id="form-dialog-title">
                <p className="text-center title-dialog">Experience</p>
              </DialogTitle>
              <DialogContent>
                <Experience
            ...
            <Dialog
              open={this.state.onChangeInfo && this.state.onChangeEducations}
              onClose={this.updateDataArray}
              aria-labelledby="form-dialog-title"
            >
              <DialogTitle id="form-dialog-title">
                <p className="text-center title-dialog">Education</p>
              </DialogTitle>
              <DialogContent>
                <Education
            ...
            <Dialog
              open={this.state.onChangeInfo && this.state.onDeleteExperiences}
              onClose={this.deleteDataArray}
              aria-labelledby="form-dialog-title"
            >
              <DialogContent>
                <DeleteExperienceControl
                  experience={selectedExperience}
                  type={EXPERIENCE_ARRAY}
            ...
            <Dialog
              open={this.state.onChangeInfo && this.state.onDeleteEducations}
              onClose={this.deleteDataArray}
              aria-labelledby="form-dialog-title"
            >
              <DialogContent>
                <DeleteEducationControl
                  education={selectedEducation}
